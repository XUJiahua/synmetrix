...
### 方案 3: 使用 Hasura Computed Fields ⭐⭐⭐

**核心思想**:
- 保留 `users` 表作为空壳
- 通过 PostgreSQL 函数动态生成用户数据
...
```sql
CREATE OR REPLACE FUNCTION get_keycloak_user_profile(user_row users)
RETURNS json AS $$
DECLARE
  keycloak_data json;
BEGIN
  -- 调用 Keycloak API（通过 plpython 或 http extension）
  SELECT content::json INTO keycloak_data
  FROM http_get(
    'http://keycloak:8080/admin/realms/synmetrix/users/' || user_row.id::text
  );
...
- table:
  ...
  computed_fields:
    - name: profile
      definition:
        function:
          name: get_keycloak_user_profile
          schema: public
      comment: Fetch user profile from Keycloak
```
...
**问题**:
- ⚠️ 需要 PostgreSQL HTTP extension (`pg_http`)
- ⚠️ 性能问题（每次查询都调用 Keycloak API）
- ⚠️ 需要处理 Keycloak API 错误
...
SELECT DISTINCT ON (user_id)
  ...
  updated_at
FROM (
  SELECT user_id, created_at, updated_at FROM datasources
  UNION
  SELECT user_id, created_at, updated_at FROM members
  UNION
  SELECT user_id, created_at, updated_at FROM alerts
  -- ... 其他表
) AS all_users;
```
...
**问题**:
- ❌ Hasura 不支持视图上的外键关系
- ❌ 无法使用 `array_relationships` (datasources, members 等)
- ❌ 无法插入数据到视图
- ❌ 性能问题（UNION 多个大表）
